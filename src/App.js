...
import "easymde/dist/easymde.min.css"   // todo 这个是哪里来的css,暂时先注释
import './App.css';
...
function App() {
  ...
  // remote.app.getPath('documents') 拿到文档的地址
  ...
  const deleteFile = (id) => {
    if (files[id].isNew) {
      ...
      // 和 delete files[id]的区别，视频里开始用的delete后面改为上面这种了
      // 用delete是同一个对象，不会触发setState更新
    ...
    remote.dialog.showOpenDialog({
      ...
      if (Array.isArray(paths)) {
        ...
        const importFilesArr = filteredPaths.map(path => {
          return {
            id: uuidv4(),
            title: basename(path, extname(path)),  // 拿到去除后缀名的文件名称
            path,
          }
        })
